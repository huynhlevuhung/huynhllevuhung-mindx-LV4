...
export default function UserLayout() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (Array.isArray(res.data)) {
        ...
        data = res.data.data; // fallback
      }

      setUsers(data);
      setFilteredUsers(data);
    } catch (err) {
      // show useful error in console
  ...
  // Unique roles + provinces from data
...
const handleUpdateUser = async (id, data) => {
  try {
    ...
    const updatedUser = res.data?.data?.user; // lấy đúng field từ response

    setUsers((prev) =>
      prev.map((u) => (u._id === id ? updatedUser : u))
    );

    setEditingUser(null); // đóng modal đúng state
  } catch (err) {
...
const handleAddUser = async (data) => {
  try {
    ...
    setAddingUser(false); // đóng modal
  } catch (err) {
    console.error("Thêm user thất bại:", err?.response?.data ?? err);
    throw err; // 🔹 ném lỗi để AddUserModal bắt
  }
};
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <img
              src={
                u.avatar
                  ? `http://localhost:3000/uploads/${u.avatar}`
                  : defaultAvatar
              }
              alt="avatar"
